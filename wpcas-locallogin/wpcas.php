<?php
/*
Plugin Name: wpCAS with Local Login Option
Version: 1.0
...
Author: Casey Bisson Modified by Les Leach
...
if( !is_array( $wpcas_options )){
	$wpcas_options = get_option( 'wpcas_options_locallogin' );
...
if ($cas_configured) {
	...
	//set the local login session variable if Local Login is allowed
	if ($wpcas_options['allow_local_login'] AND isset($_GET['localauth'])){
	  setcookie('wpcaslocalauth', '1');
	  $_COOKIE['wpcaslocalauth'] = '1';
	}
	
	if ($wpcas_options['allow_local_login'] AND isset($_GET['wpcasauth'])){
          setcookie('wpcaslocalauth', NULL, time() - 10000);
          unset($_COOKIE['wpcaslocalauth']);
	}
 }
...
//add_action('login_form_login', array('wpCAS', 'authenticate'), 10, 2);
add_action('login_footer', array('wpCAS', 'wpcasloginlink'));
add_action('wp_logout', array('wpCAS', 'logout'));
if (!$wpcas_options['allow_local_login']) {
	...
	*/
	function authenticate() {
		...
		if( phpCAS::isAuthenticated() ){
			...
			//see if we are allowing local authentication
		  if ($wpcas_options['allow_local_login'] AND isset($_COOKIE['wpcaslocalauth'])){
		               return true; //pass through to regular WP login
			} 
			// hey, authenticate with CAS
	...
	//add local login over-ride link (will take the user to cas authentication)
	function wpcasloginlink(){
	...
	function show_password_fields( $show_password_fields ) {
		...
		return $wpcas_options['allow_local_login'];
		//return false;
	}
	...
	function check_passwords( $user, $pass1, $pass2 ) {
		...
		if ($wpcas_options['allow_local_login'] AND (strlen($pass1) OR strlen($pass2))){
			return; //we are creating/editing a local user
		}
		//cas user will need randomly generated password!
...
function wpcas_options_page() {
	...
	$optionarray_def = array(
				 'new_user' => FALSE,
				 'redirect_url' => '',
				 'email_suffix' => 'swarthmore.edu',
				 'cas_version' => CAS_VERSION_2_0,
				 'include_path' => 'CAS/CAS.php',
				 'server_hostname' => 'login.swarthmore.edu',
				 'server_port' => '8443',
				 'server_path' => '/cas',
	...
	if (isset($_POST['submit']) ) {		 
		...
		update_option('wpcas_options_locallogin', $optionarray_update);
	}
	
	// Get Options
	$optionarray_def = get_option('wpcas_options_locallogin');
	...
	<table width="700px" cellspacing="2" cellpadding="5" class="editform">
		<tr valign="center"> 
			<th width="300px" scope="row">CAS version</th> 
	...
	<?php
		/*local login option added by Les Leach on 6/30/2011 */
	?>
	<h4><?php _e( 'Local Login Option', 'wpcas' ) ?></h4>
	<table width="700px" cellspacing="2" cellpadding="5" class="editform">
		<tr valign="center"> 
			<th width="300px" scope="row">Allow local login</th> 
			<td><select name="allow_local_login" id="allow_local_login_swataddin">
				<option value="1" <?php echo ($optionarray_def['allow_local_login'])?'selected':''; ?>>Yes</option>
				<option value="0" <?php echo (!$optionarray_def['allow_local_login'])?'selected':''; ?>>No</option>
			</td>
		</tr>
		<tr>
			<td colspan="2">
				Selecting "Yes" for local login will allow users to login by both CAS and regular WordPress login
			</td>
		</tr>
	</table>
	<div class="submit">
